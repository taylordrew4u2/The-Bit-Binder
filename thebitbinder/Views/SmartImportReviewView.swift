...
struct SmartImportReviewView: View {
            ...
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    if let rateLimitError = viewModel.rateLimitError {
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 8, style: .continuous)
                                .fill(Color.bitbinderAccent.opacity(0.9))
                        )
                        .padding(.horizontal, 12)
                        .padding(.top, 4)
                    }
                    ...
                    if viewModel.autoAcceptedCount > 0 {
                        ...
                        .background(
                            RoundedRectangle(cornerRadius: 8, style: .continuous)
                                .fill(Color.bitbinderAccent.opacity(0.85))
                        )
                        .padding(.horizontal, 12)
                        .padding(.top, 4)
                    }
    ...
    private func jokeCard(_ item: ImportReviewItem) -> some View {
        VStack(alignment: .leading, spacing: 16) {
                    ...
                    HStack(spacing: 6) {
                        ForEach(item.editedTags, id: \.self) { tag in
                            Text(tag)
                                ...
                                .background(
                                    Capsule().fill(
                                        roastMode ? Color.bitbinderAccent.opacity(0.2) : .accentColor.opacity(0.1)
                                    )
                                )
                                .foregroundColor(Color.bitbinderAccent)
                        }
            ...
            HStack(spacing: 0) {
                ...
                .foregroundColor(Color.bitbinderAccent.opacity(0.5))
            }
        }
        .padding(20)
    ...
    private var swipeBorderColor: Color {
        ...
        if dragOffset.width > 30 { return Color.bitbinderAccent.opacity(opacity) }
    ...
    private func importTipRow(icon: String, text: LocalizedStringKey) -> some View {
        HStack(alignment: .top, spacing: 10) {
            Image(systemName: icon)
                .font(.system(size: 14))
                .foregroundColor(roastMode ? Color.bitbinderAccent.opacity(0.8) : .accentColor)
                .frame(width: 20)
            ...
            VStack(spacing: 28) {
                ...
                HStack(spacing: 40) {
                    ...
                    VStack(spacing: 10) {
                        ZStack {
                            Circle()
                                .fill(Color.bitbinderAccent.opacity(0.2))
                                .frame(width: 60, height: 60)
                            Image(systemName: "hand.point.right.fill")
                                .font(.system(size: 24))
                                .foregroundColor(Color.bitbinderAccent)
